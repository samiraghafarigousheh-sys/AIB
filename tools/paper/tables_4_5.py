...
-----------------------
This generator was written on ``claude/aib-canonical-clean-weather-0fr0mp`` at
commit f5a5229, which is not an ancestor of ``claude/new-session-carh9p``, so
Tables 4 and 5 had no copy in this branch's paper tree. It is restored verbatim;
only the invocation differs, because the trajectory now lives at
``trajectory_v2/`` rather than ``canonical_trajectory/`` and the closure ref has
to be pinned so the wind-profile commit is not swept into the instrument:

    python tools/paper/tables_4_5.py \
        --weather weather_cache/AUS_VIC_Melbourne-Essendon.Fields.958660_TMYx.2011-2025.epw \
        --raw results/paper/trajectory_v2/trajectory_raw.json \
        --outdir results/paper/tables_4_5 \
        --closure-base 978db37 --closure-ref 0935730

Table 5b needs the isolation branches, so fetch them first:

    git fetch origin '+refs/heads/*:refs/remotes/origin/*'

NOTE ON WHICH BUILDING DICTIONARY 5b USES. The 2x2 states are branch engines,
but ``closed_balance_six_state`` reads ``build_bui`` from REPO_ROOT/examples, not
from the worktree. So 5b measures old engines against the CURRENT building
definition -- which is why its numbers moved when the q50 band was recalibrated,
and why that movement is not a wind-profile effect. Recorded in
results/paper/SUPERSEDED_wind_profile.md section 7.
"""
