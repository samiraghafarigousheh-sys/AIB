...
Weather: `AUS_VIC_Melbourne-Essendon.Fields.958660_TMYx.2011-2025.epw`. The h_ce correlation is driven by the **wind local to the wall** — the station column × 0.6574 for terrain and height.
...
**100.0 % of the +1.75 kWh comes from hours with real, non-zero wind**, and the above-pivot band alone carries -1.24 kWh (-70.5 %) over 2,578 hours. The exactly-zero bucket is now 138 hours and +0.00 kWh, 0.0 % of the total.

## The mechanism

With only 29.4 % of hours above the pivot the dynamic coefficient sits **below** the ISO constant on the year (mean 16.73 against 20 W/(m²·K)), so the exposed west wall is coupled *less* tightly to outdoor air than ISO 13789's frozen value assumes. ISO 13789 §9.5 freezes 4 m/s, which is a met-station speed; a wall in suburban terrain three storeys up does not see it.
...
Of the hours the dynamic model adds to the cooling plant (11), the mean wind is 2.74 m/s and 0.0 % read exactly zero.
